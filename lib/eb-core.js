...
eb.comm = {
    ...
    getUoHandle: function(apiKey, uoId, uoType){
        ...
        return uoType === undefined ?
            sprintf("%s00%08x", apiKey, eb.misc.inputToHexNum(uoId)) :
            sprintf("%s00%08x00%08x", apiKey, eb.misc.inputToHexNum(uoId), eb.misc.inputToHexNum(uoType));
    },

    /**
     * Parses handle string to its components.
     * @param {String} handle
     * @returns {Array} parsed handle
     */
...
eb.comm.connector.prototype = {
    ...
    configure: function(configObject){
        ...
        if ("host" in configObject){
            var p = eb.misc.parseUrl(configObject.host);
...
eb.client = {
     ...
     * Process data call.
     * @param options
     * @param {Array} [options.retry] configuration or retry handler.
    ...
    processData: function(options){
        ...
        this.configure = function(options){
            ...
            this.request.configure(options);

            // Logger
            this.logger = options.logger || function(){};

            // Configure retry handler.
            this.retryHandler = new RetryHandler(ebextend(true, {maxAttempts: 1}, options.retry || {}));
            ...
            return new Promise((function(resolve, reject) {
                ...
                var onEbOpFailure = (function(data){
                    if (this.retryHandler.limitReached()){
                        ...
                        reject(ebextend(data, {retry:{'reason':'retry attempts limit reached'}}));
                        return;
                    }
     ...
     * Create UO call.
     * @param options
     * @param {Array} [options.tpl]
     * @param {Array} [options.keys]
     * @param {Array} [options.retry]
     * @param {Function} [options.logger]
     */
    createUO: function(options){
        ...
        this.configure = function(options){
            ...
            // Configure retry handler.
            this.retryHandler = new RetryHandler(ebextend(true, {maxAttempts: 0}, options.retry || {}));

            return this;
        };

        /**
         * Constructs createUO request.
         * Returns promise on the result.
         *
        ...
        this.call = function(data){
            return new Promise((function(resolve, reject) {
                ...
                var onEbOpFailure = (function(data){
                    if (this.retryHandler.limitReached()){
                        this.logger("EB failure - limit reached");
                        reject(ebextend(data, {retry:{'reason':'retry attempts limit reached', phase:1}}));
                        return;
                    }
        ...
        this.getTpl_ = function(onSuccess, onFail){
            ...
            this.tplRequest.configure(this.config);
            ...
            var uotype = eb.misc.inputToHexNum(requestData.type);
            uotype |= this.hasAppKey_() ? 1<<21 : 0;
            uotype |= this.hasCommKey_() ? 1<<20 : 0;
            requestData.type = uotype;

            // Callbacks settings.
            this.tplRequest.done(function(response, requestObj, data) {
                if (response && response.responseStatus !== undefined && response.responseStatus != eb.comm.status.SW_STAT_OK){
                    onFail(data);
                    return;
                }
        ...
        this.importUo_ = function(data, onSuccess, onFail){
            ...
            this.impRequest.configure(this.config);
        ...
        this.onTemplateFetched_ = function(response, resolve, reject){
            ...
            var onEbOpFailure = (function(data){
                if (this.retryHandler.limitReached()){
                    this.logger("EB failure - limit reached");
                    reject(ebextend(data, {retry:{'reason':'retry attempts limit reached', phase:2}}));
                    return;
                }
